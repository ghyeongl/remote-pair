...
# default mountpoints land at /Volumes/<device>/<share>, sshfs is rejected, and
...
# ── xpair-mount default mountpoint is /Volumes/<sanitized-remote-host>/<share> ──
new_sandbox
printf 'REMOTE_HOST=user@office-mac.local\n' > "$RP_CLIENT_DIR/client.env"
cat > "$MOCKBIN/mount" <<'EOF'
#!/bin/bash
echo '//alice@office-mac.local/foo on /Volumes/user_office-mac.local/foo (smbfs, nodev, nosuid, mounted by alice)'
EOF
chmod +x "$MOCKBIN/mount"
run_mount mount /Users/alice/Projects/foo
it "mount/default-volumes-path"
assert_contains "$RP_OUT" "Mountpoint: /Volumes/user_office-mac.local/foo" "default mountpoint uses /Volumes device/share scheme"
cleanup_sandbox
...
LOCAL_BIN="$MOCKBIN"
FOLDER_MAPS="/Volumes/test-host/proj::/Users/alice/proj;$SYNC_DIR::/Users/alice/sync"
FOLDER_MAP_MODES="/Volumes/test-host/proj::mount;$SYNC_DIR::sync"
PATH="$MOCKBIN:$PATH"
...
assert_contains "$RP_ERR" "could not mount /Users/alice/proj at /Volumes/test-host/proj before attach" "launch guard warns on mount failure"
cleanup_sandbox
...
cat > "$MOCKBIN/mount" <<'EOF'
#!/bin/bash
echo '//alice@test-host/proj on /Volumes/test-host/proj (smbfs, nodev, nosuid, mounted by alice)'
EOF
cat > "$MOCKBIN/xpair-mount" <<'EOF'
#!/bin/bash
{ printf 'xpair-mount'; for a in "$@"; do printf '|%s' "$a"; done; printf '\n'; } >> "$MOCKLOG"
