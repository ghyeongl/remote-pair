...
**Two layers, and the newer governs.** Sections 0–4 are the requirement corpus reconstructed from the Q-utterances (2026-06). Section −1 is the **0.6.0 engineering direction** (`roadmap-0.6.0.md`, self-declared "engineering SSoT"), which is newer and **supersedes the corpus where the supersession map says so.** Superseded rows are kept (not deleted) and annotated — a `[사실]` from an utterance stays true as history even when a later decision overrides it. Where −1 and 0–4 disagree and −1 does not explicitly supersede, that is a live contradiction to raise, not to merge.
...
| REQ-APPROVE-4 | P0-fix | [사실] | Approve router success verdict must confirm the **outcome** (which button pressed, or blocked call resumed), not merely that the dialog closed. Current `xpair-approve-router.sh` returns `success` at lines 143/153 on any key that closes the dialog (`dialog_gone` @115), so pressing Decline reads as success. **Falsifier:** a pre-existing result-confirmation path other than dialog-closure already present in the router | VP-verified live tree 2026-08-21; two sessions blocked today; see `pm-memo.md` #2 |
| REQ-APPROVE-5 | before-cutover | [사실] | Approve SKILL.md (live tree, next generation) must carry: (a) the "about to raise a dialog — arm approve before the signing command" case, since `ssh agent refused operation` dies immediately; (b) non-blocking trigger-touch vs blocking `xpair approve` kept distinct; (c) locked-vault is out of approve's scope → ask human, don't retry-loop | skill-orchestrate materials; see `pm-memo.md` #3 |
