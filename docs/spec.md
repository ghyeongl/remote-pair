...
**Consequence for the approve items:** #2's real target was the **0.4.13 `remote-pair-approve-router.sh`** (the `dialog_gone`-only success bug, pre-fix at lines 145/155, `dialog_gone()` @117), **not** the abandoned-line `xpair-approve-router.sh` 143/153 the brief named. **Fixed in PR #121 (merged).** The bug lineage was shared; the file fixed is the live one.
...
| REQ-APPROVE-4 | DONE (#121) | [파생] | Approve router success verdict must confirm the **outcome** (which key candidate was declared/pressed), not merely that the dialog closed. **Fixed in PR #121 (merged 2026-08-21):** the live `host/remote-pair-approve-router.sh` now validates declared key candidates before accepting a close (key-path success return relocated). *Pre-fix* it returned `success` on any key that closed the dialog, so Decline read as success. **Falsifier (regression guard):** the router accepting a close by an undeclared/Decline key | Derived from: CEO symptom ("approve가 동작을 잘 안한다") + VP direct-read verification 2026-08-21 + skill-orchestrate report; two sessions blocked. NOT Q-corpus-backed. See `pm-memo.md` #2 |
| REQ-APPROVE-5 | MOSTLY-DONE (verify) | [파생] | Approve skill guidance must carry: (a) "arm approve before the signing command" ordering, since `ssh agent refused operation` dies immediately; (b) non-blocking fallback (`~/.remote-pair/bin/approve` / trigger touch) vs blocking `remote-pair approve` kept distinct; (c) locked-vault out of scope → ask human, don't retry-loop. **The live 0.4.13 `host/skills/approve/SKILL.md` already documents (a) and (b)** (lines 46–47); the CLI is `remote-pair approve` (not `xpair approve`). **Residual (bigger than a delta): the skill references a `~/.remote-pair/bin/approve` helper that does NOT exist on 0.4.13** (verified — only the `touch` trigger path might work, and only if a watcher consumes it; unverified). Fix the skill's dead reference, verify the trigger-touch path actually has a watcher, then diff installed vs repo copy and add recurrence-prevention; confirm (c) is present. | Was derived from skill-orchestrate's peer report, whose "content missing" premise proved FALSE against the live skill on direct read. NOT Q-corpus-backed. See `pm-memo.md` #3 |
| REQ-APPROVE-PERSIST | open | [봉인] | Persist auto-detection is intentionally **NOT** a requirement unless the user reopens it | Q0108 |
...
*(Out-of-band fixes: REQ-APPROVE-4 is a P0 correctness fix on a shipped path — done, merged in PR #121. REQ-APPROVE-5 is a P3 follow-up content port into the live 0.4.13 approve skill — see `pm-memo.md` #3. Neither belongs to the roadmap.)*
