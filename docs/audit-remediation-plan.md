...
3. F36 — prune `onboarding-preload.cjs` to **exactly the live set, derived at implementation time**: `grep -roE 'window\.remotepair\.[a-zA-Z]+' onboarding-webview/src` (~17 distinct methods at e3750c8d — mapping, pairing, host install/update, and consent calls are all live; do NOT trust any hardcoded count from the audit). **Bridge implementations of `sshReachable`/`hostPermissions`/`hostEngineStatus`/`hostEnvEngine` stay** (used by onboarding-main's launch guard); update `per-mapping-method-readback.test.js:104`.
...
**Constraint**: version-skew — an old client against a new host (or vice versa) must still connect. **There is no existing RD protocol-version negotiation to lean on** (`shared/screen-protocol/constants.json` is transport constants only; neither `serve_webrtc.rs` nor `remote-desktop.js` exchanges a version), so step 0 of this WS is to add one: carry a protocol version/capability field in the existing signaling exchange (offer/answer JSON), register it in `shared/screen-protocol` so the SoT guard covers it, negotiate old-style channels when the peer omits it, and drop the fallback one release later.
