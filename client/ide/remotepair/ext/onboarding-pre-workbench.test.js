...
test("Q0369 Q0421 Q0424 Q0426 onboarding is a consumed-sentinel pre-workbench BrowserWindow", () => {
  ...
  assert.match(onboardingMain, /const clientEnv = readClientEnv\(\)[\s\S]*const host = configuredRemoteHost\(clientEnv\)[\s\S]*if \(!host\) return START_STEP\.WELCOME[\s\S]*cliProbeReady\(probeBridge\)[\s\S]*if \(process\.platform !== 'darwin'\)[\s\S]*return null/);
  assert.match(onboardingMain, /cliProbeReady\(probeBridge\)[\s\S]*probeBridge\.sshReachable\(host\)[\s\S]*probeBridge\.hostAppStatus\(host\)[\s\S]*probeBridge\.hostPermissions\(\{ host \}\)[\s\S]*configuredHostEngine\(host, probeBridge\)[\s\S]*probeBridge\.hostEngineStatus\(engineToCheck\)/);
