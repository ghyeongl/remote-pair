...
test("Q0369 Q0421 Q0424 Q0426 onboarding is a consumed-sentinel pre-workbench BrowserWindow", () => {
  ...
  assert.match(onboardingMain, /probeBridge\.cliReady\(\)[\s\S]*probeBridge\.sshReachable\(host\)[\s\S]*probeBridge\.hostAppStatus\(host\)[\s\S]*probeBridge\.hostPermissions\(\{ host \}\)[\s\S]*configuredHostEngine\(host, probeBridge\)[\s\S]*probeBridge\.hostEngineStatus\(hostEngine\)/);
